...
export default function RightTextBox({ details, isFull, isSpread } : any) {
  return (
    <div className='mt-8 w-full md:w-[55%] flex flex-col justify-center items-center pr-4 md:pr-[128px] relative'>
      ...
      {details?.subTitle ? <h2 className="text-lg md:text-xl font-bold text-black mb-2">{details.subTitle}</h2> : null }
      {isSpread ? <div className='bg-[#f0f5ff] p-2 rounded-xl mb-1 w-full md:w-[250px]'>
        <h2 className="text-xs md:text-base font-bold text-[#2a406b] text-center">{details?.subTitleSupport}</h2>
      </div> : null }
      {details?.description ? <h3 className="text-xs md:text-sm text-center text-black font-semibold mb-2">{details.description}</h3> : null }
